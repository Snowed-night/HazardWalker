...
- 对深度为 `flat` 或横纵曲率明显不一致的 `anisotropic` 近圆候选，轨迹状态会直接标为 `needs_reobservation`（而不是普通
  `tentative`）；展示字段同样会置 `requires_reobservation=true`。这使圆柱正面在实际侧向复查前
  不会被误读成已识别红球。
- 单视角球面深度检查同时比较水平、竖直和两条对角线四个方向的凸曲率；任意角度的圆柱侧面、弧形板等单轴曲面不能
  提供 `spherical` 正证据。轴向深度采样不足时保持 `unknown`，由主动视角策略继续复查。
...
## 11.  附录：2026-07-15 黄亚锌整理材料索引

以下文档由黄亚锌在 2026-07-15 基于姜晨等同学的算法设计与实验数据整理：

| 文档 | 路径 | 说明 |
|---|---|---|
| **实验结果表** | `reports/perception/docs/维护感知组实验结果表.md` | 五类实验的专业论文格式结果表（场景/帧数/真值/检出/漏检/误检/置信度/动态/备注） |
| **开发文档与算法总结** | `docs/groups/perception/开发文档与算法总结.md` | 已解决问题、使用算法（HSV检测/三维定位/多帧确认/主动视角/动态记录）、参数速查 |
| **五大模块材料** | `docs/groups/perception/五大模块材料.md` | 红球检测/动态识别/主动视角/多帧确认/三维定位的完整梳理 |
| **全面测试案例** | `docs/groups/perception/全面测试案例及验证鲁棒性.md` | 基于姜晨算法设计的 L1-L5 五层测试体系（含已实现 122 项 + 建议新增 ~70 项） |
