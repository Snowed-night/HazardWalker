...
在远程 RDP 桌面的浏览器打开 `http://127.0.0.1:6081/hazardwalker.html`，该专用页面会自动连接，
且仅保留按浏览器视口铺满的 Gazebo 画面。需要真正全屏时再按浏览器 `F11`。
该页面刻意禁用 VNC 键盘/鼠标输入，避免浏览器焦点误触 Gazebo；机器狗仍由独占终端的 ROS2 键盘节点控制。
