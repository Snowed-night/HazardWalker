...
新增代码文件时，需要在文件首部注明本文件的作用，建议包含所属小组、文件职责、当前实现边界和验证方式。重要模块、重要类和重要函数需要有简明注释或 docstring，说明输入、输出、关键逻辑和后续扩展点。注释应帮助成员理解代码，不写重复代码本身含义的空泛说明。代码注释和 docstring 不使用短横线作为项目符号，需要分点时使用中文编号、中文冒号或直接换行说明。
...
感知基线已从单纯 HSV 红色像素统计升级为 OpenCV 辅助的红球候选筛选：先做 HSV 红色阈值分割，再通过连通域、面积、圆度、bbox 长宽比和面积比例过滤红色方块及不规则物体误检。当前策略优先降低虚警，已新增 `localize_hazard.py`、`track_hazards.py` 和 `detection_metrics.py`，并在感知节点中接入 CameraInfo、对齐深度图和 TF；真实三维定位效果仍需在主力机 Gazebo/官方平台中验证 RGB、深度和 TF 的对齐情况。
