...
## usage
After starting the controller, the robot will lie on the ground of the
simulator. Press `2` to switch from **Passive** to **FixedStand**, then press
`4` to switch to **Trotting**. The HazardWalker maintainer mapping is:
`w` forward, `s` backward, `a` turn left, `d` turn right, and `k` stop
immediately. Space remains a legacy stop alias.
