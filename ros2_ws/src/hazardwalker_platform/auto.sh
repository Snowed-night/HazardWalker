...
# 解除暂停后控制器先固定站立 CONTROLLER_AUTO_STAND_DELAY_SEC，再等待
# CONTROLLER_AUTO_RL_DELAY_SEC 才进入 RL。探针必须覆盖这段状态切换，不能
# 在固定站立阶段误判控制链失效。
CONTROLLER_RL_SETTLE_SEC="${CONTROLLER_RL_SETTLE_SEC:-10.0}"
...
CATKIN_DEVEL="$WORKSPACE_DIR/.ros1_catkin_ws/devel"
if [ ! -f "$CATKIN_DEVEL/setup.bash" ]; then
  CATKIN_DEVEL="$WORKSPACE_DIR/devel"
fi
source "$CATKIN_DEVEL/setup.bash"
...
if [ -d "$GAZEBO_CLASSIC_PLUGIN_DIR" ]; then
  export GAZEBO_PLUGIN_PATH="$CATKIN_DEVEL/lib:/opt/ros/noetic/lib:$GAZEBO_CLASSIC_PLUGIN_DIR:${GAZEBO_PLUGIN_PATH:-}"
...
if [ "${SIMENV_PRACTICE:-0}" = "1" ]; then
  "$CATKIN_DEVEL/lib/building_obstacles/generate_competition_scene.py" "${GENERATOR_ARGS[@]}" \
...
# ROS1 bridge 是传感器接口，不应被控制器后续 RL/probe 的诊断结果阻塞。此处
# Gazebo 已解除暂停，先确认最新里程计与时钟，再将官方传感器暴露给 ROS2。
if [ "$START_ROSBRIDGE" = "1" ]; then
  echo "Starting rosbridge after Gazebo unpause..."
  timeout "$CONTROLLER_READY_TIMEOUT_SEC" rostopic echo -n 1 /Odometry_gazebo >/dev/null || {
    echo "Odometry did not become available; refusing rosbridge startup." >&2
    exit 1
  }
  timeout "$CONTROLLER_READY_TIMEOUT_SEC" rostopic echo -n 1 /clock >/dev/null || {
    echo "Simulation clock did not become available; refusing rosbridge startup." >&2
    exit 1
  }
...
if [ "$START_CONTROLLER" = "1" ] && [ "$SIMENV_AUTO_RL" = "1" ]; then
  # 前面的 HEADLESS_FSM 和 /cmd_vel 订阅门禁已经确认控制状态；当前控制器
  # 版本不再稳定输出旧的 “Switched from fixed stand to RL” 文本。以这句旧
  # 日志作为第二门槛会在物理探针已通过后错误退出主进程，并触发 Docker
  # 重启，从而让整个 ROS 时间域回跳。保留下面的真实位移和机身高度探针。
