...
def test_frontier_delegates_local_avoidance_to_unitree_move_base():
    ...
    assert 'Unitree move_base command unavailable, stale, or idle' in source
...
def test_official_corridor_completion_uses_the_same_control_frame_as_motion():
    ...
    assert "'strict_room_orientation_min_speed', 0.60" in source
    assert 'not pure_inspection_orientation' in source
    orient_branch = source.split(
